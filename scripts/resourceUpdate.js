...
function Update(Met_PerHour, Cry_PerHour, Deu_PerHour, Met_Max, Cry_Max, Deu_Max, Met_MaxOver, Cry_MaxOver, Deu_MaxOver, Met_Cur, Cry_Cur, Deu_Cur)
{
	...
	{
		Met_PerSec = Met_PerHour / 3600;
		IntMet_PerSec = intval(Met_PerSec);
		Met_Rest = Met_PerSec - IntMet_PerSec;
		Metal += IntMet_PerSec;
		Metal_Rest += Met_Rest;
		if(Metal_Rest > 1)
		{
	...
	{
		Cry_PerSec = Cry_PerHour / 3600;
		IntCry_PerSec = intval(Cry_PerSec);
		Cry_Rest = Cry_PerSec - IntCry_PerSec;
	...
	{
		Deu_PerSec = Deu_PerHour / 3600;
		IntDeu_PerSec = intval(Deu_PerSec);
		Deu_Rest = Deu_PerSec - IntDeu_PerSec;
...
$(document).ready(function()
{
	...
	$('.resMet:not(#resMet)').hover(function(){ ResElements['met'].mouseover(); }, function(){ ResElements['met'].mouseout(); });
	
	ResElements['cry'].qtip($.extend(
	{
		content: CToolTip,
		position:
		{
	...
	$('.resCry:not(#resCry)').hover(function(){ ResElements['cry'].mouseover(); }, function(){ ResElements['cry'].mouseout(); });
	
	ResElements['deu'].qtip($.extend(
	{
		content: DToolTip,
		position:
		{
	...
	$('.resDeu:not(#resDeu)').hover(function(){ ResElements['deu'].mouseover(); }, function(){ ResElements['deu'].mouseout(); });
	
	ResElements['eng'].qtip($.extend(
	{
		content: EToolTip,
		position:
		{
	...
	$('.resEnr:not(#resEng)').hover(function(){ ResElements['eng'].mouseover(); }, function(){ ResElements['eng'].mouseout(); });
});