...
class PHPMailer {
 ...
 /**
   ...
   * @param string $address
   * @return boolean true on success, false if address doesn't exists
   */
  public function RemoveToAddress($address) {
     if (isset($this->all_recipients[strtolower($address)])) {
        foreach($this->to as $MasterKey => $Data){
            if($Data[0] == $address){
                unset($this->to[$MasterKey]);
                break;
            }
        }
        unset($this->all_recipients[strtolower($address)]);
        return true;
     } else {
        return false;
     }
