...
{
	$Return = array();
	if($FleetRow['fleet_start_time'] <= $Now AND $FleetRow['fleet_mess'] == 0 AND $FleetRow['fleet_mission'] != 2)
	{
		...
		if(in_array($FleetRow['fleet_mission'], array(1, 5, 6, 7, 8, 9)))
		{
			...
			if($FleetRow['fleet_mission'] == 9 AND $FleetRow['fleet_target_owner'] > 0)
			{
				$Return['taskData'][] = $FleetRow['fleet_target_owner'];
			}
		}
		if($FleetRow['fleet_mission'] == 1 AND $FleetRow['fleets_count'] > 0)
		{
	...
	if($FleetRow['fleet_end_stay'] > 0 AND $FleetRow['fleet_end_stay'] <= $Now AND $FleetRow['fleet_mess'] != 2)
	{
		$Return['timeSort'][$FleetRow['fleet_end_stay'].str_pad($FleetRow['fleet_id'], 20, '0', STR_PAD_LEFT).'02'] = 2;
	}
	if($FleetRow['fleet_end_time'] <= $Now)
	{
		if(($FleetRow['fleet_mission'] == 4 AND !empty($Return['timeSort'])) OR $FleetRow['fleet_mission'] == 10)
		{
