...
function GalaxyRowUser($GalaxyRowPlanet, $GalaxyRowUser, $MyBuddies, $SFBStatus)
{
    ...
    if (
        !isset($GalaxyRowPlanet['id_owner']) ||
        $GalaxyRowPlanet['id_owner'] <= 0
    ) {
        if ($_User['settings_Galaxy_ShowUserAvatars'] == 1) {
            return implode('', [ renderEmptyGalaxyCell(), renderEmptyGalaxyCell(), renderEmptyGalaxyCell() ]);
        }

        return '<th class="hiFnt" colspan="2">&nbsp;</th><th class="hiFnt">&nbsp;</th>';
    }
    ...
    $isUserBanned = isUserBanned($GalaxyRowPlanet);
    $isUserVacation = isOnVacation($GalaxyRowPlanet);
