$(document).ready(function()
{
	...
	function duration()
	{
		ret = Math.round((35000 / speed() * Math.sqrt(distance() * 10 / maxspeed()) + 10) / speedFactor());
		FlightDuration = ret;
	...
	function consumption()
	{
		...
		for(i = 200; i < 300; i += 1)
		{
			ThisConsumption = $('#consumption'+i).val();
			if(ThisConsumption !== undefined && ThisConsumption !== '')
			{
				spd = 35000 / (dur * speedfactor - 10) * Math.sqrt(dist * 10 / parseInt($('#speed'+i).val(), 10));
	...
	function createTimeCounters()
	{
		...
		var CMonths = CTimeCounter.getMonth();
