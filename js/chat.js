...
var Messages = {users: {}, msgCount: 0, firstID: 0, lastID: 0, lastGet: 0};
...
{
	var MsgOpt = [], AddLastMarker = '', MsgDate = new Date((MessageData.d + ServerStamp) * 1000);
	if(UserAuth >= 90)
	{
	...
	MsgDate = padTime(MsgDate.getHours())+':'+padTime(MsgDate.getMinutes())+', '+padTime(MsgDate.getDate())+' '+MonthsLang[(MsgDate.getMonth() + 1)]+' '+MsgDate.getFullYear();
	return '<tr id="sbmsg_'+MessageData.id+'" class="doFadeIn '+AddLastMarker+'"><th class="usr aright" valign="top" nowrap><a href="#" class="usrRef usrColor_'+Messages.users[MessageData.u].c+'" title="'+JSLang['usrRef']+'">'+Messages.users[MessageData.u].n+'</a> <a href="profile.php?uid='+MessageData.u+'" class="usrColor_'+Messages.users[MessageData.u].c+'" target="_blank" title="'+JSLang['profile']+'">#</a></th><th class="msg aleft">'+MessageData.t+'</th><th class="dt aright" valign="top" nowrap>('+MsgDate+')</th>'+MsgOpt+'</tr>';
}
// > Add Smiles
...
{
	var ReqParam = null, ReCalc_FirstID = false, ReCalc_LastID = false, UpdateLastGetTime = false;
	if(Messages.init === false)
	{
		ReqParam = {init: true, rid: RoomID};
	}
	else
	{
		...
		if(reqStatus == 'success')
		{
			...
			if(Response.onl != null)
			{
				...
				for(var i in Response.onl)
				{
					...
					NewOnlineUsers.push('<a href="#" class="usrRef usrColor_'+SplitText[2]+' '+((SplitText[3] != null && SplitText[3] == 1) ? 'usrInv' : '')+'" title="'+JSLang['usrRef']+'">'+SplitText[1]+'</a> <a href="profile.php?uid='+SplitText[0]+'" class="usrColor_'+SplitText[2]+'" target="_blank" title="'+JSLang['profile']+'">#</a>');
			...
			if(InAppError !== true)
			{
				...
				{
					Messages = {users: {}, msgCount: 0, firstID: 0, lastID: 0, lastGet: 0};
					ShoutBoxTable.html('');
				}
				else
				{
...
{
	if(typeof ThisErrorMsg === 'undefined'){ThisErrorMsg = '&nbsp;';}
	if(typeof FadeChat === 'undefined'){FadeChat = false;}
	if(typeof LockControl === 'undefined'){LockControl = false;}
	if(typeof NoUnThrow === 'undefined'){NoUnThrow = false;}
	...
	ErrorBox.html(ThisErrorMsg).fadeIn(400, function()
	{
		if(NoUnThrow === false)
		{
...
$(document).ready(function()
{
	...
	YourNickname = preg_quote(YourNickname);
	YourNickname_RegExp = new RegExp(YourNickname, 'gi');
	YourNicknameReplace_RegExp = new RegExp('('+YourNickname+')', 'gi');
	...
	.on('click', '.delMsg', function()
	{
		...
		ThisID = $(this).parent().parent().attr('id').replace('sbmsg_', '');
		ActivityBox.show(0);
	...
	{
		ThisID = $(this).parent().parent().attr('id').replace('sbmsg_', '');
		ThisUID = $(this).parent().parent().find('th.usr > a:not(.usrRef)').attr('href').replace('profile.php?uid=', '');
	...
	$('.setChg').change(function()
	{
		...
		if($(this).attr('type') == 'checkbox')
		{
			...
			RevertChange = function(){ThisElement.prop('checked', !ThisElement.is(':checked'));};
			
			if(ThisPost['setChg_1'] != null)
			{
				if(ThisPost['setChg_1'] === true)
				{
					OnSuccess = function(){$('#onlineBox_you').addClass('usrInv')};
				}
				else if(ThisPost['setChg_1'] === false)
				{
					OnSuccess = function(){$('#onlineBox_you').removeClass('usrInv')};
				}
			}
		}
		else
		{
			...
			RevertChange = function(){ThisElement.val('');};
		}
		...
		.success(function(data)
		{
			...
			SettingsInterval = setTimeout(function(){SettingsBox.fadeOut(500);}, 1000);
		})
		.error(function()
		{
