		...
		.complete(function(response, requestStatus)
		{
			if(requestStatus == 'success')
			{
				...
				if(CodeText.indexOf('replace_target') >= 0){
					...
					ReplaceTextNow = CodeText.replace('replace_target', '['+Galaxy+':'+System+':'+Planet+']'+AddReplace);
...
{	
	...
	.on('click', '.icoMissile', function()
	{
		...
		if(UseAjax === true)
		{
			...
			GetPos = RegExp.exec($(this).attr('href'));
			...
			if(ThisPos != CurrentMissilesPos)
			{
				...
				{
					MissilesPos.fadeOut('fast', function(){MissilesPos.html(CurrentMissilesPos);$(this).fadeIn('fast');});
				}
				else
				{
